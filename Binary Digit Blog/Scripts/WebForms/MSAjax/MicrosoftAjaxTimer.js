...
Sys.UI._Timer = function(a) {
    Sys.UI._Timer.initializeBase(this, [a]);
...
Sys.UI._Timer.prototype = {
    ...
    dispose: function() {
        this._stopTimer();
        if (this._pageRequestManager !== null)this._pageRequestManager.remove_endRequest(this._endRequestHandlerDelegate);
        Sys.UI._Timer.callBaseMethod(this, "dispose");
    },
    _doPostback: function() { __doPostBack(this.get_uniqueID(), ""); },
    _handleEndRequest: function(c, b) {
        var a = b.get_dataItems()[this.get_id()];
        if (a)this._update(a[0], a[1]);
    ...
    _startTimer: function() { this._timer = window.setTimeout(Function.createDelegate(this, this._raiseTick), this.get_interval()); },
    _stopTimer: function() {
        if (this._timer !== null) {
    ...
    _update: function(c, b) {
        var a = !this.get_enabled(), d = this.get_interval() !== b;
        if (!a && (!c || d)) {
            this._stopTimer();
            a = true;
        }
        this.set_enabled(c);
        this.set_interval(b);
        if (this.get_enabled() && a)this._startTimer();
    }
};
